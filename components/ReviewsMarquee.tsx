  ...
  return (
    <section aria-label="Customer reviews" aria-live="off">
      <div className="group relative flex overflow-hidden rounded-3xl border border-border/60 bg-background shadow-soft">
        ...
        <div className="flex min-w-full animate-marquee items-center gap-6 py-6 transition-[animation-play-state] group-hover:[animation-play-state:paused]">
          {doubled.map((review, index) => (
            <figure
              key={`${review.name}-${index}`}
              className="w-64 shrink-0 rounded-2xl border border-border/40 bg-muted/40 p-4 text-sm text-muted-foreground"
            >
